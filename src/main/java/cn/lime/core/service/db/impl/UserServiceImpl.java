...
import cn.lime.core._constant.YesNoEnum;
...
import cn.lime.core._media.oss.QiNiuOssService;
import cn.lime.core._sms.UniSmsService;
import cn.lime.core._wx.auth.WxMpOuterService;
import cn.lime.core._wx.bean.WxPhoneInfo;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    @Resource
    private UniSmsService phoneService;
    @Resource
    ...
    public LoginVo bindPhoneByPhoneCode(String phoneCode) {
        // 获取手机号
        WxPhoneInfo wxPhoneInfo = wxMpOuterService.getWxPhoneInfo(coreParams.getWxProperties().getWxMpAppId(),
                coreParams.getWxProperties().getWxMpSecretId(), phoneCode);
    ...
    public LoginVo doBindPhone(String phone) {
        ...
        // 之前有注册过
            ...
            Pair<String, String> tokenPair = accessTokenHandler.generateToken(personnel.getUserId(),
                    personnel.getRole(), ReqThreadLocal.getInfo().getPlatform(), ReqThreadLocal.getInfo().getIp());
            Long refreshTtl = redisTemplateMap.get(RedisDb.TOKEN.getVal()).opsForValue().getOperations().getExpire(tokenPair.getValue());

            return new LoginVo(personnel.getUserId(), tokenPair.getKey(), tokenPair.getValue(), accessTokenHandler.getAccessTokenExpire(), refreshTtl);
        }
    }
